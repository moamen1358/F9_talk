...
F9 ↑  →  ⌨    Text appears at cursor   (~150 ms with AssemblyAI U3 Pro · ~250 ms with Deepgram)
```
...
| **Right-click → Cloud provider ▶** | Switch between AssemblyAI and Deepgram on the fly — backend rebuilds without restart |
| **Right-click → API Keys…** | Paste keys; saving hot-reloads the active backend |
...
f9-talk speaks to two streaming STT providers; pick either or both, switch any time from the tray.

| Provider | Default model | Latency (warm) | Notes |
|---|---|---|---|
| **AssemblyAI** *(default)* | Universal-3 Pro Streaming | ~150 ms P50 | Lowest end-of-press → typed-text latency. New in v0.4. |
| **Deepgram** | Nova-3 | ~250–300 ms | Persistent WS, auto-reconnect on close. |

Both have free tiers. f9-talk auto-selects whichever key is set in `~/.config/F9_talk/secrets.env` — AssemblyAI takes priority when both are present.

- AssemblyAI: [www.assemblyai.com](https://www.assemblyai.com/) — free credit
...
- **Two cloud providers** with hot-swap from the tray (AssemblyAI default, Deepgram alt).
- **Local offline backend** via [`whisper-rs`](https://github.com/tazz4843/whisper-rs) (whisper.cpp). `--features cuda` for GPU; CPU works on any box.
...
- **Auto-reconnect** — Deepgram + AssemblyAI WS closes are recovered transparently with exponential backoff (1 s → 30 s cap).
...
| Command | Description |
|---|---|
| `f9-talk` | Cloud STT, auto-pick provider, type at cursor |
| `f9-talk --cloud-provider assemblyai` | Force AssemblyAI |
| `f9-talk --cloud-provider deepgram` | Force Deepgram |
...
| `f9-talk-stt` | `Stt` trait + AssemblyAI Universal-3 Pro Streaming, Deepgram Nova-3, whisper.cpp local. |
...
| At least one cloud key | AssemblyAI or Deepgram — both have free tiers |
...
[MIT](LICENSE) — Moamen Ghareeb. No proprietary or paid components in the project itself; the cloud STT providers (AssemblyAI / Deepgram) and the optional MyMemory translation API have their own pricing tiers.
